            ...
            {[...Array(totalColumns)].map((ignored, column) => (
                <TableRow>
                    {[...Array(totalDayOfWeek)].map((ignored2, day) => {
                        ...
                        if ((column === 0 && day < dayOfFirstDayDate - 1) || currentDay > informationMonth.total)
    ...
    return (
        <TableContainer>
            <Table>
                <TableHead>
                    <TableRow>
                        {Utils.daysOfWeek.map(day => <TableCell key={`HeadTableMonth-${day.name}`} >{day.name}</TableCell>)}
                    </TableRow>
                </TableHead>
